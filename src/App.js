...
import AfterHarvest from "./components/AfterHarvest/AfterHarvest";
...
function App() {
  ...
  const [gains, setGains] = useState(initialCapitalGains);
  ...
  const handleToggle = (index) => {
    ...
    const newGains = JSON.parse(JSON.stringify(initialCapitalGains)); // deep clone
    newSelected.forEach((i) => {
      const h = holdingsData[i];
      if (h.stcg.gain > 0) newGains.stcg.profits += h.stcg.gain;
      else newGains.stcg.losses += Math.abs(h.stcg.gain);

      if (h.ltcg.gain > 0) newGains.ltcg.profits += h.ltcg.gain;
      else newGains.ltcg.losses += Math.abs(h.ltcg.gain);
    });
  ...
  return (
    <div className="App">
      <div className="container">
        <img className="image" src="./media/images/logo2.svg" alt="logo" />
      </div>
      ...
      <div className="harvest-container">
        <PreHarvest data={sampleData} />
        <AfterHarvest gainsData={gains} savings={savings} />
      </div>
      <div>
        <HoldingsTable
